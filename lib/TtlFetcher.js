...
function getLovTtl(query, cb) {
  const lovInfoQuery = 'http://lov.okfn.org/dataset/lov/api/v2/vocabulary/info?vocab=' + encodeURIComponent(query);
  request.get({url: lovInfoQuery, json: true}, function (err, res, lovResult) {
    if (err) {
      return cb(err);
    }
    if (!lovResult.uri) {
      debug('Cannot find uri for ' + query + ' in LOV');
      return cb();
    }
    let ttlUrl = null;
...
TurtleFetcher.prototype.fetchByUrl = function (url, cb) {
  ...
  getLovTtl(url, function (err, lovUrl, ttl) {
    if (err) {
      return cb(err);
    }
    if (ttl) {
...
function getTtl(url, cb) {
  ...
  async.series([
    function (done) {
      debug('first try accept ttl/owl ' + url);
